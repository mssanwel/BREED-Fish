...
void loop() {
  ...
  char cmd[11];
  if (Serial1.available() > 0) {  //if more than 0 bytes available to read in the buffer
    ...
    //after reading 1 complete signal, empty the entire buffer in an attempt to improve responsiveness
    clear_buffer();
    check = checkSum(siglen, cmd);
  }
  ...
  if (check == true) {
    ...
    write_to_servo();  //write the signals to the servo
    encoder();    //reads the current position of the shaft
    check_tail_pos(); //check encoder position and manipulate power to motor if needed
    killswitch();
    //write the power to motor
    write_to_motor(pwm_Pin, motor_Pwm);
  } else {
...
void write_to_motor(int pin, int pwm_val) {
  analogWrite (pin, pwm_val);
  Serial.println("motor pwm: " + pwm_val);
  //update the turn signals
  oldTurnVal = turnVal;
  oldPower = power;
  Serial.print("Motor:" + pwm_val);
  motor_temp = pwm_val;
...
void killswitch() {
  if ((millis() - tailTimer1) > 3000) {
    motor_Pwm = 0;
    Serial.print("kill switch motor_Pwm: ");
